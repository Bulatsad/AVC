...
namespace AVI
{
    class MUL : IAsseblerVirtualModule
    {
        ...
        private List<byte> MULR(string to)
        {
            List<byte> result = new List<byte>();
            result.Add(BaitCodeList["mulr"]);
            result.Add(RegisterCodes[to]);
            return result;
        }
        private List<byte> MULM(string to)
        {
            List<byte> result = new List<byte>();
            result.Add(BaitCodeList["mulm"]);
        ...
        public byte[] Compile(string instruction)
        {
            ...
            byte rubbish;
            if (args[0][0] == '[' && args[0][args[0].Length - 1] == ']') binaryinst = MULM(args[0]);
            else if (RegisterCodes.TryGetValue(args[0], out rubbish)) binaryinst = MULR(args[0]);
        ...
        {
            return baitCode == BaitCodeList["mulr"] ||
                baitCode == BaitCodeList["mulm"];
        }
    }
}